...
CREATE VIEW V_ENTREE AS
SELECT PRODUITID,SUM(QUANTITE) AS TOTALENTREE FROM MouvementStock WHERE QUANTITE>0 GROUP BY PRODUITID;

CREATE VIEW V_SORTIE AS
SELECT PRODUITID,SUM(QUANTITE) AS TOTALSORTIE FROM MouvementStock WHERE QUANTITE<0 GROUP BY PRODUITID;

SELECT VE.PRODUITID,COALESCE(VE.TOTALENTREE, 0) ,COALESCE(VS.TOTALSORTIE, 0) FROM V_ENTREE VE JOIN V_SORTIE VS ON VE.PRODUITID=VS.PRODUITID;
