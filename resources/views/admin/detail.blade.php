                      ...
                      for ($semaine = 1; $semaine <= 6; $semaine++) {
                        ...
                        for ($jour = 1; $jour <= 7; $jour++) {
                          ...
                          if ($semaine == 1 && $jour < date('N', strtotime($annee_affiche . '-' . $mois_affiche . '-01'))) {
                            ...
                            // if()
                            if (count($arr) > 0) {
                              echo '<td id="ppl"  style="width:110px;height:100px">' . '<a href="/planningcollect/test?date=' . $da . '">' . $compteur_jours . '</a>';
                              $o = 0;
                              foreach ($arr as $key) {
                                // # code...
                                echo '<div class="box">' . $key->getProduit()->nom . ":" . $key->tonnage . 'T' . '</div>';
                                // if()
                                $o++;
                                if ($o >= 1) {
                                  if (count($arr) > 1) {
                              ...
                              echo '<td>' . '<a href="/planningcollect/test?date=' . $da . '">' . $compteur_jours . '</a></td>';
                            }
                            $compteur_jours++;
                          }
                        }
